...
def get_args_parser():
    ...
    parser.add_argument('--track_test_split', default='test', type=str)
    parser.add_argument('--track_thresh', default=0.2, type=float)
...
#         base_ds = get_coco_api_from_dataset(dataset_val)
    ...
    if args.eval:
        dataset_val = build_dataset(image_set=args.track_test_split, args=args)
        if args.distributed:
            if args.cache_mode:
        ...
        res_tracks = evaluate(model, criterion, postprocessors, data_loader_val,
                                                          device, args.output_dir, tracker=tracker, 
...
#             utils.save_on_master(coco_evaluator.coco_eval["bbox"].eval, output_dir / "eval.pth")
            if res_tracks is not None:
                ...
                for _, info in dataset_val.coco.imgs.items():
                    ...
                    if args.dataset_file == "MOVText":
                        video_name = info["file_name"].split("/")[0] + "_" + info["file_name"].split("/")[1]
                    else:
...
#                 save_motval(res_tracks, args.output_dir, video_to_images, video_names)
                save_track(res_tracks, args.output_dir, video_to_images, video_names, args.track_test_split,det_val=args.det_val)
