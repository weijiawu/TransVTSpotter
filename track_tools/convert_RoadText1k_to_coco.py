...
#         box, rotate = get_rotate(points)
        box = np.array([object_boxes[0], object_boxes[2], object_boxes[1], object_boxes[3]])
        rotate = 0
        if content == "###":
            continue
...
#             continue
        bboxes, IDs, rotates = \
            getBboxesAndLabels_icd13( annotation[frame_id])   
...
if __name__ == '__main__':
    ...
    DATA_PATH = '/share/wuweijia/Data/VideoText/RoadText1k'
    ...
    for split in SPLITS:
        if split == "test":
            ...
            ann_path_ = os.path.join(DATA_PATH, 'Ground_truths/Test_json')
#             seq_list = get_list(os.path.join(DATA_PATH,"test_list.txt"))
            
        else:
            data_path = os.path.join(DATA_PATH, 'images/train')
            ann_path_ = os.path.join(DATA_PATH, 'Ground_truths/Json')
...
#         seqs = seq_list
        ...
        ann_cnt = 0
        video_cnt = 0
        for idx,seq in enumerate(sorted(seqs)):
            if '.DS_Store' in seq or 'train' in seq:
                continue
            if idx>3:
                break
...
#                 continue
            ...
            ann_path = os.path.join(ann_path_, seq+".json")
