...
#             continue
        ...
        if quality == "LOW":
            continue
#         if "?" in Transcription or "55" in Transcription:
#             continue            
...
if __name__ == '__main__':
    ...
    DATA_PATH = '/share/wuweijia/Data/VideoText/minetto'
    ...
    for split in SPLITS:
        if split == "test":
            data_path = os.path.join(DATA_PATH, 'minetto_test')
            ann_path_ = os.path.join(DATA_PATH, 'minetto_xml')
        else:
            data_path = os.path.join(DATA_PATH, 'minetto_test')
            ann_path_ = os.path.join(DATA_PATH, 'minetto_xml')
        ...
        for seq in sorted(seqs):
            ...
            for i in range(num_images):
                img = cv2.imread(os.path.join(data_path, '{}/{}.jpg'.format(seq, str(i).zfill(6))))
#                 print(os.path.join(data_path, '{}/{:06d}.jpg'.format(seq, i + 1)))
                height, width = img.shape[:2]
                image_info = {'file_name': '{}/{}.jpg'.format(seq, str(i).zfill(6)),  # image name.
